"""
Admin panel — a simple password-protected web UI served at /admin
Lets you view players, override needs, manage moodlets, and control the server.
"""

from fastapi import APIRouter, Depends, HTTPException, Request, Form
from fastapi.responses import HTMLResponse
from app.database import get_db
from app.services.needs import update_need, apply_moodlet
...
def check_admin(request: Request):
    """Simple secret-based admin auth via query param or header."""
    cfg = get_config()
...
def admin_style():
    ...
    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .badge-yellow { background: #3a3a1a; color: #ffeb3b; }
      .badge-purple { background: #2a1a3a; color: #ce93d8; }
      .card { background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 10px; padding: 1rem 1.25rem; margin-bottom: 1rem; }
...
def bar_html(value: float) -> str:
    ...
    if pct > 60:
        ...
        color = "#f44336"
    return f'<div class="bar-wrap"><div class="bar-fill" style="width:{pct}%;background:{color};"></div></div> {pct:.1f}'


@router.get("/", response_class=HTMLResponse)
async def admin_home(request: Request, db: aiosqlite.Connection = Depends(get_db)):
    check_admin(request)
    secret = request.query_params.get("secret", "")
    cfg = get_config()

    async with db.execute("SELECT COUNT(*) as c FROM players WHERE is_banned = 0") as cur:
        total = (await cur.fetchone())["c"]
    async with db.execute("SELECT COUNT(*) as c FROM players WHERE is_online = 1 AND is_banned = 0") as cur:
        online = (await cur.fetchone())["c"]
    async with db.execute("SELECT COUNT(*) as c FROM event_log") as cur:
        events = (await cur.fetchone())["c"]
    async with db.execute("SELECT COUNT(*) as c FROM moodlets WHERE expires_at IS NULL OR expires_at > datetime('now')") as cur:
        active_moodlets = (await cur.fetchone())["c"]

    async with db.execute("SELECT * FROM players WHERE is_banned = 0 ORDER BY last_seen DESC") as cur:
        players = await cur.fetchall()

    players_html = ""
    for p in players:
        pid = p["id"]
        async with db.execute("SELECT need_key, value FROM needs WHERE player_id = ?", (pid,)) as cur:
            needs = {r["need_key"]: r["value"] for r in await cur.fetchall()}
        ...
        need_bars = " ".join([
            f'<span title="{k}">{bar_html(v)}</span>'
...
async def admin_player(
    ...
    async with db.execute("SELECT * FROM needs WHERE player_id = ?", (player_id,)) as cur:
        needs = await cur.fetchall()

    async with db.execute("SELECT * FROM skills WHERE player_id = ?", (player_id,)) as cur:
        skills = await cur.fetchall()

    async with db.execute(
        "SELECT * FROM moodlets WHERE player_id = ? AND (expires_at IS NULL OR expires_at > datetime('now'))",
        (player_id,)
    ) as cur:
        moodlets = await cur.fetchall()

    async with db.execute(
        "SELECT * FROM event_log WHERE player_id = ? ORDER BY timestamp DESC LIMIT 30",
        (player_id,)
    ) as cur:
        ...
        <tr>
          ...
          <td>
            <form method="post" action="/admin/player/{player_id}/set_need?secret={secret}">
              ...
              <input type="number" name="value" value="{n['value']:.1f}" min="0" max="100" step="0.1" style="width:70px;">
    ...
    skills_rows = "".join([
        f"<tr><td>{s['skill_key']}</td><td>Level {s['level']}</td><td>{s['xp']:.1f} XP</td></tr>"
        for s in skills
    ])

    moodlet_rows = "".join([
        f"<tr><td>{m['moodlet_key']}</td><td>{'negative' if m['is_negative'] else 'positive'}</td><td>{m['expires_at'] or 'permanent'}</td></tr>"
        for m in moodlets
    ]) or "<tr><td colspan='3' style='color:#666;'>No active moodlets</td></tr>"

    log_rows = "".join([
        f"<tr><td style='color:#888;font-size:0.75rem;'>{l['timestamp']}</td><td>{l['need_key'] or '—'}</td><td>{l['action_text']}</td><td style='color:{'#4caf50' if l['delta'] >= 0 else '#f44336'};'>{'+' if l['delta'] >= 0 else ''}{l['delta']}</td></tr>"
        for l in logs
    ])
    ...
    <form method="post" action="/admin/player/{player_id}/apply_moodlet?secret={secret}">
      <select name="moodlet_key">
        {''.join([f'<option value="{k}">{k}</option>' for k in cfg['moodlets'].keys()])}
      </select>
...
async def set_need(
    ...
    await db.execute(
        ...
        (value, player_id, need_key)
    )
    await db.execute(
        "INSERT INTO event_log (player_id, need_key, action_text, delta, value_after) VALUES (?, ?, ?, ?, ?)",
        (player_id, need_key, f"Admin override: {need_key} set to {value}", 0, value)
    )
    await db.commit()
...
async def admin_apply_moodlet(
    player_id: int,
    request: Request,
    moodlet_key: str = Form(...),
    db: aiosqlite.Connection = Depends(get_db)
):
    check_admin(request)
    secret = request.query_params.get("secret", "")
